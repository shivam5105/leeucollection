...
if(!class_exists('multiLevelCustomPosts'))
{
	class multiLevelCustomPosts
	{
	    ...
	    public function addParentBox($parent_slug, $box_title, $box_context = 'side', $box_priority = 'default')
	    {
	        ...
	        add_action('add_meta_boxes', array($this, 'addingCustomMetaBoxes'), 10, 2);
	    }
	    public function addingCustomMetaBoxes()
	    {
			add_meta_box($this->slug."-parent", $this->box_title, array($this, 'renderCustomMetaBox'), $this->slug, $this->box_context, $this->box_priority);
	    ...
	    {
			add_filter("post_type_link", array($this, "updatePostPermalinks"), 10, 3);
	    }
		public function updatePostPermalinks($permalink, $post, $leavename)
		{
			$post_id = $post->ID;
			if($post->post_type != $this->slug || empty($permalink) || in_array($post->post_status, array("draft", "pending", "auto-draft")))
			{
			 	return $permalink;
			}
...
function registerMyCustomPost()
{
	...
	$args = array(
		...
		"supports"		=> array( "title", "revisions", "thumbnail", "editor", "page-attributes", "custom-fields"/*, "post-formats"*/ )
	);
	$parentSlug = "hotel";	
...
{
	$data = array('post-slug' => 'hotel', 'name' => 'Locations' ,'singular_name' => 'Location', 'slug' => 'location-category', 'textdomain' => 'leeucollection');
